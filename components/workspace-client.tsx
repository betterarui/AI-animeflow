...
export function WorkspaceClient({ projectId }: { projectId: string }) {
  ...
  if (loading) {
    return (
      <main className="cinematic-app-page workspace-cinematic-page flex min-h-screen items-center justify-center">
        <CinematicAppBackdrop variant="workspace" />
  ...
  if (!project) {
    return (
      <main className="cinematic-app-page workspace-cinematic-page flex min-h-screen items-center justify-center px-6">
        <CinematicAppBackdrop variant="workspace" />
  ...
  return (
    <main className="cinematic-app-page workspace-cinematic-page min-h-screen bg-paper">
      <CinematicAppBackdrop variant="workspace" />
        ...
        <section className="min-w-0">
          ...
          {activeStep === "video" && (
            <VideoStep
              ...
              onRegenerateVideo={(storyboardId) => void startTask("/api/generation/videos", { projectId, storyboardId }, "video", "videos")}
              reviewing={isTaskBusy("review")}
...
function VideoStep(props: {
  ...
  onRegenerateVideo: (storyboardId: string) => void;
  reviewing: boolean;
  generatingImages: boolean;
            ...
            {props.storyboards.map((shot) => (
              <article key={shot.id} className="overflow-hidden rounded-card border border-line bg-white">
                ...
                <div className="space-y-3 p-4">
                  ...
                  <button
                    type="button"
                    onClick={() => props.onRegenerateVideo(shot.id)}
                    disabled={!props.canUseVideo || !shot.imageUrl || props.generatingVideos}
                    className="inline-flex items-center gap-2 rounded-card border border-line bg-white px-3 py-2 text-sm font-semibold text-ink disabled:opacity-45"
                  >
                    {props.generatingVideos ? <Loader2 className="animate-spin" size={15} /> : <Play size={15} />}
                    {props.generatingVideos ? "生成中" : "重生成视频"}
                  </button>
                </div>
              </article>
            ))}
...
function ImagePreviewModal({ preview, onClose }: { preview: ImagePreview; onClose: () => void }) {
  return (
    <div className="cinematic-image-preview-overlay fixed inset-0 z-[60] flex items-center justify-center bg-ink/80 p-4" onClick={onClose}>
      <div
        className="cinematic-image-preview-panel flex max-h-[92vh] w-full max-w-6xl flex-col overflow-hidden rounded-card bg-white shadow-work"
      ...
      >
        ...
        <div className="cinematic-image-preview-body flex min-h-0 flex-1 items-center justify-center bg-ink p-4">
...
function Modal({ title, children, onClose }: { title: string; children: React.ReactNode; onClose: () => void }) {
  return (
    <div className="cinematic-modal-overlay fixed inset-0 z-50 flex items-center justify-center bg-ink/45 p-4">
      <div className="cinematic-modal-panel max-h-[90vh] w-full max-w-2xl overflow-y-auto rounded-card bg-white shadow-work">
