...
- 用户注册、登录和当前用户识别。
- 数据库一次性邀请码注册、邀请码使用记录、管理员账号、用户启停和每日生成额度控制。
...
- 管理员可通过 `/admin/usage` 查看用户、项目数、今日用量，并调整用户状态和每日额度。
...
# Deprecated: static shared invite codes are no longer used. Generate one-time invite codes in /admin/usage.
INVITE_CODES=""
...
- `REGISTRATION_MODE=invite` 时，新用户需要填写管理员在 `/admin/usage` 生成的一次性邀请码才能注册；`.env` 中的 `INVITE_CODES` 已停用。
- `ADMIN_EMAILS` 用英文逗号配置管理员邮箱，管理员可访问 `/admin/usage`，且不受每日额度限制。
- `DEFAULT_DAILY_QUOTA` 是普通用户注册后的默认每日生成点数；管理员后续可在用量管理页单独调整。
...
公网开放后，为了防止模型 token 被过度消耗，当前项目已内置应用级访问控制：

- 注册默认采用邀请码模式：`REGISTRATION_MODE="invite"`。
- 邀请码由管理员在 `https://animeflow.cn/admin/usage` 生成，存入数据库并且只能使用一次；后台可查看创建人、绑定邮箱、使用人和使用时间，也可再次复制未使用的邀请码。
- 邀请码被使用或撤销后会清空完整码，只保留短标识和使用记录；本次改造前已生成的旧邀请码无法恢复完整码。
- `.env` 的 `INVITE_CODES` 已停用，不再作为有效注册凭证，避免共享码被多人重复使用。
- 允许注册邮箱白名单配置在 `ALLOWED_REGISTRATION_EMAILS`。
- 管理员邮箱配置在 `ADMIN_EMAILS`，多个邮箱用英文逗号分隔。
- 普通用户每日额度来自 `users.daily_quota`，新用户默认值来自 `DEFAULT_DAILY_QUOTA`。
- 管理员可访问 `https://animeflow.cn/admin/usage` 管理用户状态、角色和每日额度。
- 被停用用户无法继续登录或发起生成任务。
- 管理员不受每日额度限制，适合用于测试、排障和运营管理。
...
点数会按当天创建的 `generation_tasks` 统计，失败任务也会计入，因为失败调用也可能已经消耗模型额度。
...
- `GET/POST/PATCH /api/admin/invite-codes`
